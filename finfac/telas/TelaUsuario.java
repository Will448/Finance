...
public class TelaUsuario extends javax.swing.JInternalFrame {
    ...
    private void consultar() {
        String sql = "SELECT id AS, nome, email, datanascimento, parentesco, cpf, fone, login, perfil FROM usuarios WHERE nome = ?";
    ...
    private void adicionar() {
        String sql = "insert into usuarios(nome,email,datanascimento,parentesco,cpf,fone,login,senha,perfil) values(?,?,?,?,?,?,?,?,?)";
        try {
            pst = conexao.prepareStatement(sql);
    ...
    private void limpar() {
       ...
       private void pesquisar_usuario() {
           String sql = "SELECT id, nome, email, datanascimento, parentesco, cpf, fone, login, perfil " +
    ...
    private void setarCampos() {
        ...
        // Define os campos de texto e combo box
        txtUsuNome.setText(tblUsuarios.getValueAt(setar, 1) != null ? tblUsuarios.getValueAt(setar, 1).toString() : "");
        txtUsuEmail.setText(tblUsuarios.getValueAt(setar, 2) != null ? tblUsuarios.getValueAt(setar, 2).toString() : "");
        txtUsuDataNascimento.setText(tblUsuarios.getValueAt(setar, 3) != null ? tblUsuarios.getValueAt(setar, 3).toString() : "");
        txtUsuCpf.setText(tblUsuarios.getValueAt(setar, 5) != null ? tblUsuarios.getValueAt(setar, 5).toString() : "");
        txtUsuFone.setText(tblUsuarios.getValueAt(setar, 6) != null ? tblUsuarios.getValueAt(setar, 6).toString() : "");
        txtUsuLogin.setText(tblUsuarios.getValueAt(setar, 7) != null ? tblUsuarios.getValueAt(setar, 7).toString() : "");
        txtUsuSenha.setText(tblUsuarios.getValueAt(setar, 8) != null ? tblUsuarios.getValueAt(setar, 8).toString() : "");

        // Configura os valores dos ComboBox
        Object parentesco = tblUsuarios.getValueAt(setar, 4);
        cboUsuParentesco.setSelectedItem(parentesco != null ? parentesco.toString() : cboUsuParentesco.getItemAt(0));

        Object perfil = tblUsuarios.getValueAt(setar, 9);
        cboUsuPerfil.setSelectedItem(perfil != null ? perfil.toString() : cboUsuPerfil.getItemAt(0));
    ...
    private void alterar() {
        int confirma = JOptionPane.showConfirmDialog(null, "Confirmar as alterações nos dados deste cliente?", "Atenção!", JOptionPane.YES_NO_OPTION);
        if (confirma == JOptionPane.YES_OPTION) {
            String sql = "update usuarios set nome=?,email=?,datanascimento=?,parentesco=?,cpf=?,fone=?,login=?,senha=?,perfil=? where nome=?";
            try {
                pst = conexao.prepareStatement(sql);
                ...
                pst.setString(8, txtUsuSenha.getText());
                pst.setString(9, cboUsuPerfil.getSelectedItem().toString());
                pst.setString(10, txtUsuNome.getText()); // WHERE nome=?
    ...
    private void initComponents() {
        ...
        cboUsuPerfil.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "admin", "usuario", " " }));
