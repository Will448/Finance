...
public class TelaReceita  extends javax.swing.JInternalFrame {
    ...
    private void cadastrar() {
        String sql = "insert into tbreceitasevento(datareceita, horareceita, categoria, descricao, valor, meiopagamento, observacoes, situacao, id_evento) values (?,?,?,?,?,?,?,?,?)";
        try {
            pst = conexao.prepareStatement(sql);
    ...
    private void pesquisar() {
        String entrada = JOptionPane.showInputDialog("Digite o Número da Receita ou a Descrição:");
        String sql = "SELECT * FROM tbreceitasevento WHERE idreceita = ? OR descricao LIKE ?";
    ...
    private void alterar() {
        int confirma = JOptionPane.showConfirmDialog(null, "Confirma as alterações nas receitas deste evento ?", "Atenção!", JOptionPane.YES_NO_OPTION);
        if (confirma == JOptionPane.YES_OPTION) {
            String sql = "update tbreceitasevento set datareceita=?,horareceita=?,categoria=?,descricao=?,valor=?,meiopagamento=?,observacoes=?,situacao=? where idreceita=?";
            try {
                pst = conexao.prepareStatement(sql);
                pst.setString(1, txtData.getText());
                pst.setString(2, txtHorario.getText());
                ...
                pst.setString(7, txtObservacoes.getText());
                pst.setString(8, cboSituacao.getSelectedItem().toString());
                pst.setString(9, txtReceita.getText());
                if ((txtId.getText().isEmpty()) || (txtValor.getText().isEmpty()) || cboCategoria.getSelectedItem().equals(" ") || cboMeio.getSelectedItem().equals(" ")) {
                    JOptionPane.showMessageDialog(null, "Preencha todos os campos obrigatórios");
                } else {
                    int adicionado = pst.executeUpdate();
    ...
    private void excluir() {
        int confirma = JOptionPane.showConfirmDialog(null, "Tem certeza que deseja excluir essa receita ?", "Atenção", JOptionPane.YES_NO_CANCEL_OPTION);
        if (confirma == JOptionPane.YES_OPTION) {
            String sql = "delete from tbreceitasevento where idreceita=?";
            try {
                pst = conexao.prepareStatement(sql);
                pst.setString(1, txtReceita.getText());
                                            ...
                                            .addGroup(jPanel1Layout.createSequentialGroup()
                                                    ...
                                                    .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                                                            ...
                                                            .addGroup(jPanel1Layout.createSequentialGroup()
                                                                    ...
                                                                    .addComponent(txtReceita, javax.swing.GroupLayout.PREFERRED_SIZE, 138, javax.swing.GroupLayout.PREFERRED_SIZE))))
                                            .addGroup(jPanel1Layout.createSequentialGroup()
