...
public class TelaGastos extends javax.swing.JInternalFrame {
    ...
    private void cadastrar() {
        String sql = "insert into tbgastosevento(datagasto, horagasto, categoria, descricao, valor, meiopagamento, observacoes, situacao, id_evento) values (?,?,?,?,?,?,?,?,?)";
        try {
            pst = conexao.prepareStatement(sql);
    ...
    private void pesquisar() {
        String entrada = JOptionPane.showInputDialog("Digite o Número do Gasto ou a Descrição:");
        String sql = "SELECT * FROM tbgastosevento WHERE idgasto = ? OR descricao LIKE ?";

        try {
            pst = conexao.prepareStatement(sql);
            pst.setString(1, entrada);
            pst.setString(2, "%" + entrada + "%"); // Busca parcial por descrição
            ...
            if (rs.next()) {
                ...
                JOptionPane.showMessageDialog(null, "Gasto não encontrado!");
            }
        } catch (SQLSyntaxErrorException e) {
            JOptionPane.showMessageDialog(null, "Erro de sintaxe na consulta SQL!");
